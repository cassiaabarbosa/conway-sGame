...
class SilverGVC: UIViewController {
    ...
    func newGrid() {
        for i in alive {
            let node = setUpSquare(scene: self.scene, width: (self.view.frame.size.width / 420), heigth: (self.view.frame.size.width / 420), color: UIColor.red, positionX: Double(nodesMatrix[i.0][i.1].position.x), positionY: Double(nodesMatrix[i.0][i.1].position.y))
