...
class GoldGVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        cameraNode.position = SCNVector3(x: 6, y:-2, z: 30)
    ...
    func setUpCone(scene: SCNScene, width: CGFloat, heigth: CGFloat, color: UIColor, positionX: Double, positionY: Double) -> SCNNode{
        var geometry:SCNGeometry
        geometry = SCNSphere(radius: 0.5)
        //            geometry = SCNBox(width: width , height: heigth, length: 1.0, chamferRadius: 0.25)
        let cone = SCNNode(geometry: geometry)
        geometry.firstMaterial?.diffuse.contents = color
        scene.rootNode.addChildNode(cone)
        cone.position.x = Float(positionX)
        cone.position.y = Float(positionY)
        return cone
    }
    ...
    func setUpMatrix(scene: SCNScene) {
        ...
        for j in 0...10 {
            ...
            for i in 0...10 {
                ...
                let node = setUpCone(scene: scene, width: (self.view.frame.size.width / 420), heigth:(self.view.frame.size.width / 420), color: .black, positionX: Double(i) * 1.2, positionY: Double(-j) * 1.2)
                row.append(node)
            }
    ...
    func newGrid() {
        for i in alive {
            let node = setUpCone(scene: self.scene, width: (self.view.frame.size.width / 420), heigth: (self.view.frame.size.width / 420), color: UIColor.red, positionX: Double(nodesMatrix[i.0][i.1].position.x), positionY: Double(nodesMatrix[i.0][i.1].position.y))
            auxMatrix[i.0][i.1]=true
            node.geometry?.firstMaterial?.diffuse.contents = futureColor()
            if (futureColor() == UIColor.purple){
                doAudioPlayer.play()
            }
            else if (futureColor() == UIColor.systemPink){
                reAudioPlayer.play()
            }
            
            node.position.z = floor
            nodes.append(node)
        }
        floor += 1
        for j in dead {
